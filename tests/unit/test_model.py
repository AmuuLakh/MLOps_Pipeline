...
class TestDummyBatchInference:
    ...
    def test_compare_predictions_to_expected_shape(self):
        ...
        for batch_size in batch_sizes:
            ...
            assert (
                predictions.shape == expected_predictions_shape
            ...
            assert torch.all(
                (predictions >= 0) & (predictions < num_labels)
    ...
    def test_evaluate_accuracy_and_f1_score(self):
        ...
        assert (
            0.0 <= f1_weighted <= 1.0
        ), f"F1-weighted {f1_weighted} is out of range [0, 1]"
    ...
    def test_batch_inference_consistency(self):
        ...
        assert all(
            pred == results[0] for pred in results
